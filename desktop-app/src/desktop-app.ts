 ...
 * By default, the logger writes logs to the following locations:
  on Linux: ~/.config/banvault/logs/{process type}.log
  on macOS: ~/Library/Logs/banvault/{process type}.log
  on Windows: %USERPROFILE%\AppData\Roaming\banvault\logs\{process type}.log
...
switch (process.platform) {
  case 'win32':
    logLocation = '%USERPROFILE%\\AppData\\Roaming\\banvault\\logs\\main.log';
    break;
  case 'linux':
    logLocation = '~/.config/banvault/logs/main.log';
    break;
  case 'darwin':
    logLocation = '~/Library/Logs/banvault/main.log';
    break;
}
  ...
  constructor() {
    ...
    autoUpdater.on('update-available', (event, releaseNotes, releaseName) => {
      ...
      const dialogOpts = {
        ...
        title: 'New Version',
        message: 'An update for Banano Nault is available!',
    ...
    autoUpdater.on('error', message => {
      ...
      mainWindow.setTitle(`Banano Nault - ${autoUpdater.currentVersion}`); // reset title
      showUpdateErrors = false; // disable errors
      const dialogOpts = {
        type: 'error',
        buttons: ['OK'],
        title: 'Update Error',
        message: 'Something went wrong while downloading Banano Nault.',
...
function createWindow () {
  ...
  mainWindow.webContents.on('did-finish-load', function () {
    mainWindow.setTitle(`Banano Nault - ${autoUpdater.currentVersion}`);
  });
...
function sendStatusToWindow(progressObj) {
  ...
  mainWindow.setTitle(`Banano Nault - ${autoUpdater.currentVersion} - Downloading Update: ${Math.round(progressObj.percent)} %`);
}
...
if (!appLock) {
  ...
  // Refocus the window if the user attempts to open Banano Nault while it is already open
  ...
  const template: any = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'Banano Docs',
          click () { loadExternal('https://banano.cc/'); }
        },
        {
          label: 'Reddit (r/banano)',
          click () { loadExternal('https://www.reddit.com/r/banano'); }
        },
        {
